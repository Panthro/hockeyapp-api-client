...
- (instancetype) init {
    ...
    if (self) {
        self.apiClient = nil;
        self.host = @"https://rink.hockeyapp.net/api/2";
        self.username = @"";
        self.password = @"";
        self.accessToken= @"";
        self.verifySSL = YES;
        ...
        self.mutableDefaultHeaders[@"User-Agent"] = [NSString stringWithFormat:@"Swagger-Codegen/0.0.2/objc (%@; iOS %@; Scale/%0.2f)",[[UIDevice currentDevice] model], [[UIDevice currentDevice] systemVersion], [[UIScreen mainScreen] scale]];
    ...
    return @{
               @"token":
                   @{
                       @"type": @"api_key",
                       @"in": @"header",
                       @"key": @"X-HockeyAppToken",
                       @"value": [self getApiKeyWithPrefix:@"X-HockeyAppToken"]
                   },
               };
}
