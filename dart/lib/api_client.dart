...
class ApiClient {
  ...
  ApiClient({this.basePath: "https://rink.hockeyapp.net/api/2"}) {
    // Setup authentications (key: authentication name, value: authentication).
    _authentications['token'] = new ApiKeyAuth("header", "X-HockeyAppToken");
  }
  ...
  dynamic _deserialize(dynamic value, String targetType) {
    try {
      switch (targetType) {
        ...
        case 'App':
          return dson.map(value, new App());
        case 'Apps':
          return dson.map(value, new Apps());
        case 'AppsApps':
          return dson.map(value, new AppsApps());
        default:
          {
            Match match;
            if (value is List &&
